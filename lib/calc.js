function computeTotals({ items = [], laborTasks = [], laborHours = 0, laborRate = 0, overheadPct = 0, profitPct = 0, wastePct = 0 }) {
  const materialTotal = (items || []).reduce((s, it) => s + (Number(it.qty) || 0) * (Number(it.unit) || 0), 0)
  const wasteAmount = (wastePct / 100) * materialTotal
  const fallbackLabor = (Number(laborHours) || 0) * (Number(laborRate) || 0)
  const laborTotal = Array.isArray(laborTasks) && laborTasks.length > 0
    ? laborTasks.reduce((s, it) => s + (Number(it.hours) || 0) * (Number(it.rate) || 0), 0)
    : fallbackLabor
